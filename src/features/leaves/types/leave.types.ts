...
export interface LeaveRequestDetailEmployee extends AdminLeaveRequestEmployee {
  phone: string | null;
...
export interface MyLeaveBalanceResponse {
  success: boolean;
  data: { year: number; balances: LeaveBalance[] };
}
...
export interface AdminBalanceEmployee {
  id: string;
  employeeCode: string;
  firstName: string;
  lastName: string;
...
export interface AdminBalancesResponse {
  ...
  data: {
    year: number;
    employees: AdminBalanceEmployee[];
  };
  meta: {
    page: number;
    limit: number;
...
export interface AdminLeaveSummaryResponse {
  success: boolean;
  data: {
    pendingRequests: number;
    approvedThisMonth: number;
    employeesOnLeaveToday: number;
    autoRejectedThisMonth: number;
  };
}
export interface AdminCreateLeavePayload extends CreateLeaveRequestPayload {
  employeeId: string;
  reason: string;
}
export interface InitializeBalancesResponse {
  success: boolean;
  message: string;
  data: { year: number; created: number };
}
