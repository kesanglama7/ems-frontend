...
import { LeaveAssignmentDialog } from "@/features/leave-assignments/components/leave-assignment-dialog";
...
type WorkModeFilter = "ALL" | UserWorkMode;
        ...
        <TableBody>
          {employees.map((employee) => (
            <TableRow
              key={employee.id}
              data-state={
                selectedIds.includes(employee.id) ? "selected" : undefined
              }
            >
              <TableCell>
                <Checkbox
                  ...
                  onCheckedChange={(checked) => onToggle(employee.id, checked)}
                  disabled={selectionDisabled}
                />
              </TableCell>
              <TableCell>
              ...
              <TableCell>{formatJoiningDate(employee.dateOfJoining)}</TableCell>
              <TableCell>
                <StatusBadge status={employee.workMode} />
              </TableCell>
              <TableCell className="text-right">
              ...
              <div className="mt-4 grid gap-2 text-sm">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <p className="truncate">{employee.user.email}</p>
                  </div>
                  <div>
                    <StatusBadge status={employee.user.status} />
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <p>{formatJoiningDate(employee.dateOfJoining)}</p>
                  </div>
                  <div>
...
export function AdminEmployeeList() {
  ...
  function setPage(next: SetStateAction<number>) {
    setSelectedIds([]);
    updatePage(next);
  }
  ...
  const query = useMemo<EmployeeListQuery>(
    ...
    [page, debouncedSearch, statusFilter, departmentFilter, workModeFilter],
  );
  ...
  const selectionDisabled =
    employeesQuery.isFetching || searchInput.trim() !== debouncedSearch;
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {visibleSelectedIds.length > 0 && (
            <>
              ...
              >
                Manage leave access ({visibleSelectedIds.length})
              </Button>
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
        ...
        <Select
          ...
          onValueChange={(value) => {
            ...
            setWorkModeFilter(value as WorkModeFilter);
            setPage(1);
          }}
        >
          <SelectTrigger
      ...
      {employeesQuery.isPending ? (
        ...
        <>
          ...
          <EmployeeDesktopTable employees={employees} {...selectionProps} />

          <EmployeeMobileList employees={employees} {...selectionProps} />
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  size="sm"
                  disabled={
                    page >= meta.totalPages || employeesQuery.isFetching
                  }
                  onClick={() => setPage((current) => current + 1)}
                >
                  Next
      ...
      {assignmentOpen && (
        <LeaveAssignmentDialog
          employeeIds={visibleSelectedIds}
